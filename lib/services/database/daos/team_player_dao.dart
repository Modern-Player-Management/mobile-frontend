...
{
	@Query('select * from team_players where teamId = :teamId and `delete` = 0')
	Stream<List<TeamPlayer>> getTeamPlayers(String teamId);

	@Query('select * from team_players where teamId = :teamId and `delete` = 0')
	Future<List<TeamPlayer>> getAllTeamPlayers(String teamId);

	@Query('select * from team_players where teamId = :teamId and save = 1 and `delete` = 0')
	Future<List<TeamPlayer>> getSavedTeamPlayers(String teamId);

	@Query('select * from team_players where teamId = :teamId and save = 0 and `delete` = 0')
	Future<List<TeamPlayer>> getUnsavedTeamPlayers(String teamId);

	@Query('select * from team_players where teamId = :teamId and `delete` = 1')
